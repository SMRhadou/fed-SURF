...
def plotting(loss_constrained, acc, dist2Opt, loss_unconstrained, acc_unconstrained, dist2Opt_unconstrained, title):
    ...
    # plt.subplot(1,2,2)
    plt.plot(np.arange(0, 2*acc.shape[1], 2), np.mean(acc[:-1], axis=0)*100, 'b', label='U-DGD')
    plt.errorbar(np.arange(0, 2*acc.shape[1], 2), np.mean(acc[:-1], axis=0)*100, yerr=np.std(acc[:-1], axis=0)*100, fmt='b', capsize=3, alpha=0.5)
    plt.plot(np.arange(acc_unconstrained.shape[1]), np.mean(acc_unconstrained, axis=0)*100, 'r', label='DGD')
    plt.errorbar(np.arange(0, acc_unconstrained.shape[1], 10), np.mean(acc_unconstrained[:,::10], axis=0)*100, yerr=np.std(acc_unconstrained[:,::10], axis=0)*100, fmt='r', capsize=3, alpha=0.5)
