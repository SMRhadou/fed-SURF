...
from core.allObjectives import GradPenalty
...
def evaluateAsyn(model, dataset, objective_function, criterion, eps, nBOagents=1, printing=True, saveResults=False, resultPath=None, grad=False, **kwargs):
    ...
    with torch.no_grad():
        ...
        labels = labels.float().to(device)
        Graph = torch.tensor(Graph, device=device).float()
        last_layer, outsValid, indices, BOagents = model.forwardAsyn(images, labels, Graph, nBOagents, device=device, BOagents=kwargs['BOagents'])
        if epoch == 'Test':
            validloss = []
            validAccuracy = []
            for i in range(model.nLayers+1):
    ...
    return validloss, validAccuracy, (outsValid, BOagents)
